<?php
/**
 * JC Putter Cover Report Request
 * Base class for handling async report requests
 * Works in conjunction with AR controller
 * For reports on putter covers post 13th Feb, when we switched 
 * to assign new entitlement 6 subscribers a putter cover
 *
...
/**
 * JC_Putter_Cover_Report_Request_Six class.
 *
 */
class JC_Putter_Cover_Report_Request_Six extends JC_Async_Report_Request {

    protected $action = "putter_cover_report_request_six";

    protected $context = [
        'source' => 'JC Putter Cover Reports'
    ];
	...
	protected function fetch_data( JC_Async_Report $report, int $step ) : array {
		...
		$entitlements = $data_store->get_entitlements_for_number( 6, 'all', $this->per_step, $offset, '', 'new' );
...
JC_Registry::set_instance( 'putter-cover-report-six', new JC_Putter_Cover_Report_Request_Six());
$request = JC_Registry::get_instance( 'putter-cover-report-six' );